...
import {ctxRender, flushPromises, rq, rs, waitFor} from './helpers'
import {CodeRunner} from './CodeRunner'
import {SingleState} from '../../base/singles/types/SingleState'
...
describe('Single module handling', () => {
  ...
  it('Fetches a controller on mount, and reacts to the result.', async () => {
    ...
    mockAxios.mockResponse(rs({a: 'boop'}))
...
describe('Single module tasks', () => {
  ...
  it('Handles a fetching failure', async () => {
    ...
    ctxRender(<CodeRunner code={fetchFailure} />, {store})
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq('/test/', 'get', undefined))
    expect(mockAxios.request).toHaveBeenCalledTimes(1)
    mockAxios.mockError(Error('It broke'))
    await flushPromises()
  })
  it('Sets and retrieves errors', async () => {
    const errorRoundTrip = () => {
      const controller = useSingle<TestType>('test', {})
      useLayoutEffect(() => {
    ...
    ctxRender(<CodeRunner code={errorRoundTrip} />, {store})
  })
  it('Serializes to JSON', async () => {
    const errorRoundTrip = () => {
      const controller = useSingle<TestType>('test', {x: {a: 'Beep'}})
      useLayoutEffect(() => {
  ...
  it('Patches a resource', async () => {
    ...
    const result = ctxRender(<CodeRunner code={runPatch} renderResult={true} />, {store})
    ...
    await waitFor(
      ...
      'The patched data to be displayed.',
    )
  })
  it('Deletes a resource', async () => {
    const runDelete = () => {
      const controller = useSingle<TestType>('test', {endpoint: '/test/', x: {a: 'stuff'}})
      useLayoutEffect(() => {
    ...
    const result = ctxRender(<CodeRunner code={runDelete} renderResult={true} />, {store})
    expect(mockAxios.request).toHaveBeenCalledWith(rq('/test/', 'delete'))
    mockAxios.mockResponse(rs(null))
    await flushPromises()
    await waitFor(() => !!result.getByText(JSON.stringify({x: null})), 'The deletion to be apparent.')
  })

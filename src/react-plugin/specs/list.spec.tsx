...
import {ListState} from '../../base/lists/types/ListState'
...
describe('List tasks', () => {
  ...
  it('Creates list items from a paginated fetch request', async() => {
    ...
    mockAxios.mockResponse(rs({results: [{a: 'stuff', id: 1}, {a: 'things', id: 2}], count: 2, size: 20}))
    await flushPromises()
    await waitFor(
  ...
  it('Creates list items from an unpaginated fetch request', async() => {
    ...
    expect(mockAxios.request).toHaveBeenCalledTimes(1)
    mockAxios.mockResponse(rs([{a: 'stuff', id: 1}, {a: 'things', id: 2}]))
    await flushPromises()
    await waitFor(
      () => !!result.getByText(JSON.stringify([{a: 'stuff', id: 1}, {a: 'things', id: 2}])),
      'The list data is displayed.',

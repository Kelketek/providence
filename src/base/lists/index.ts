...
const addModules = <T>({globalOptions, state, makeModule, commit, entries, stateFor}: AddModulesArgs<T>): string[] => {
  ...
  for (const entry of entries) {
    ...
    const {remover, listeners} = (
      retrieveName(registryRoot, buildNestedPath(namespace, 'children'), true)
...
export function buildList<T>(globalOptions: GlobalOptions): (options: ListModuleOptions<T>) => BaseListModule<T> {

...
The value on a Patcher's :js:attr:`model <Patcher.model>` field is instantly updated, and in the background, a `patch` request is dispatched to the :js:attr:`endpoint <SingleController.endpoint>` with the data `{"hidden": true}`.

...
export const patcherFactory = <T, AttrName extends keyof T>({controller, globalOptions, attrName}: PatcherArgs<T, AttrName>): Patcher<T, AttrName> => {
  const patcher: Patcher<T, AttrName> = {
    ...
    rawSet(val: T[AttrName]) {
      ...
      if (oldVal === undefined) {
        console.error(`Cannot set undefined key on model. Attempted to set ${String(patcher.attrName)} on ${String(patcher.controller.name)}, ${JSON.stringify(patcher.controller.x)}`)
        return undefined
      }
